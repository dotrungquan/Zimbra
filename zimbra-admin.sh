...
function create_domain() {
    read -p "Nhập tên miền mới: " domain
    su - zimbra -c "zmprov cd $domain"
    echo "Tên miền $domain đã được tạo thành công."
}

function configure_mail_relay() {
    read -p "Nhập địa chỉ Mail Relay: " mailrelay
    su - zimbra -c "zmprov mcf zimbraMtaRelayHost $mailrelay"
    su - zimbra -c "zmcontrol restart"
    echo "Cấu hình Mail Relay thành công. Dịch vụ đã được khởi động lại."
}

while true; do
    echo "---- MENU CHÍNH ----"
    echo "1. Tạo tên miền mới"
    echo "2. Tạo tài khoản mail"
    echo "3. Tạo hoặc xem DKIM"
    echo "4. Phân quyền admin"
    echo "5. Đổi Logo"
    echo "6. Cấu hình Mail Relay"
    echo "0. Thoát"
